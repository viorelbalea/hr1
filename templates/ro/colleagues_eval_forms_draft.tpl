<div class="submeniu">
    ...
    {if $smarty.session.USER_ID == 1 || $smarty.session.ACCESSCEVAL == 1 || $smarty.session.ACCESSCEVAL == 3}
        <a href="./?m=colleagues-eval&o=formsDraft" class="selected">{translate label='Formulare evaluare'}</a>
        ...
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a
                            href="./?m=colleagues-eval&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&action=edit"
                ...
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSCEVAL == 2 || $smarty.session.ACCESSCEVAL == 3}
                    <td class="celulaMenuST"><a class="blue" href="#"
                                                onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa creezi un duplicat al acestui formular?'}'))window.location.href='./?m=colleagues-eval&o=evalDraft&EvalFormDraftID={$key}&action=clone'; return false;">Cloneaza
                        formular</a></td>{/if}
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSCEVAL == 2 || $smarty.session.ACCESSCEVAL == 3}
                    <td class="celulaMenuSTDR" style="text-align: center;"><a href="#"
                                                                              onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest formular?'}')) window.location.href='./?m=colleagues-eval&o=evalDraft&EvalFormDraftID={$key}&action=delete'; return false;">sterge</a>
                    </td>{/if}
            </tr>
        {/if}
    {/foreach}
