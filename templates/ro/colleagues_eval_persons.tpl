<div class="submeniu">
    ...
    {/if}
    {if $smarty.session.USER_ID != 1 || $smarty.session.ACCESSCEVAL == 2 ||  $smarty.session.ACCESSCEVAL == 3}
        <a href="./?m=colleagues-eval&o=evalPersons" class="selected">{translate label='Evaluari personale'}</a>
    {/if}
</div>
<br>
    ...
    {foreach from=$persons key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a href="./?m=colleagues-eval&o=forms&PersonID={$item.PersonID}"
