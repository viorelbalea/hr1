    ...
    {foreach from=$evalFormsDraft key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a href="./?m=training&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&action=edit" class="blue"><b>{$item.FormName}</b></a></td>
                ...
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSEVAL == 2 || $smarty.session.ACCESSEVAL == 3}
                    <td class="celulaMenuST"><a class="blue" href="#"
                                                onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa creezi un duplicat al acestui formular?'}'))window.location.href='./?m=training&o=evalDraft&EvalFormDraftID={$key}&action=clone'; return false;">{translate label='Cloneaza formular'}</a>
                    </td>
                {/if}
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSEVAL == 2 || $smarty.session.ACCESSEVAL == 3}
                    <td class="celulaMenuSTDR" style="text-align: center;"><a href="#"
                                                                              onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest formular?'}')) window.location.href='./?m=training&o=evalDraft&EvalFormDraftID={$key}&action=delete'; return false;">{translate label='sterge'}</a>
                    </td>{/if}
            </tr>
        {/if}
    {/foreach}
