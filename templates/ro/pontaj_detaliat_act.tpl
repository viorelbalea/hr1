...
<div id="layer_p_scroll">
    ...
    <table cellspacing="0" cellpadding="0" width="100%">
        ...
        {foreach from=$activities key=key item=item name=iter}
            {if $key > 0}
                ...
                <tr height="35">
                    ...
                    <td style="border-bottom: 1px solid #000000;">
                        <select id="Type_{$item.ID}" class="formstyle">
                            ...
                            {foreach from=$pontaj_types key=key2 item=item2}
                                <option value="{$key2}" {if $item.Type == $key2}selected{/if}>{$item2}</option>
                            {/foreach}
                        </select>
                    </td>
                    ...
                    {/if}
                    {if $smarty.session.USER_ID == 1}
                        <td style="border-bottom: 1px solid #000000;">{if $item.Status > 0}
                                <div id="button_reject"><a href="#"
                                                           onclick="if (confirm('{translate label='Sunteti sigur(a)?'}')) showInfo('{$smarty.server.REQUEST_URI}&action=reject&ID={$item.ID}', 'layer_p_content'); return false;"
                                                           title="{translate label='Anuleaza pontaj'}"><b>Reject</b></a></div>{else}&nbsp;{/if}</td>
                    {/if}
                </tr>
            {/if}
        {/foreach}
        ...
        <tr height="35">
            ...
            {if $smarty.session.USER_ID == 1}
                <td>{if $activities|@count > 2 && $activities.0.MaxStatus > 0}
                        <div id="button_reject"><a href="#"
                                                   onclick="if (confirm('{translate label='Sunteti sigur(a)?'}')) showInfo('{$smarty.server.REQUEST_URI}&action=reject_all', 'layer_p_content'); return false;"
                                                   title="{translate label='Anuleaza tot'}"><b>Reject</b></a></div>{else}&nbsp;{/if}</td>
            {/if}
        </tr>
    </table>
</div>
