...
<div class="filter">
    ...
    <select id="search_for" nume="search_for" class="cod">
        ...
        <option value="FormName" {if $smarty.get.search_for == 'FormName'}selected{/if}>{translate label='Nume formular'}</option>
    </select>
    <input type="text" id="keyword" name="keyword" value="{$smarty.get.keyword|default:''}" size="20" maxlength="30" class="cod"></td>
    <input type="button" value="Cauta" class="cod"
           onclick="window.location.href = './?m=candidates-eval&o=formsDraft&PersonID=' + document.getElementById('PersonID').value + '&search_for=' + document.getElementById('search_for').value + '&keyword=' + escape(document.getElementById('keyword').value) + '&res_per_page=' + document.getElementById('res_per_page').value"></td>
    ...
    {foreach from=$evalFormsDraft key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a href="./?m=candidates-eval&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&action=edit" class="blue"><b>{$item.FormName}</b></a>
                </td>
                ...
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSCEVAL == 2 || $smarty.session.ACCESSCEVAL == 3}
                    <td class="celulaMenuST"><a class="blue" href="#"
                                                onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa creezi un duplicat al acestui formular?'}'))window.location.href='./?m=candidates-eval&o=evalDraft&EvalFormDraftID={$key}&action=clone'; return false;">Cloneaza
                        formular</a></td>{/if}
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSCEVAL == 2 || $smarty.session.ACCESSCEVAL == 3}
                    <td class="celulaMenuSTDR" style="text-align: center;"><a href="#"
                                                                              onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest formular?'}')) window.location.href='./?m=candidates-eval&o=evalDraft&EvalFormDraftID={$key}&action=delete'; return false;">sterge</a>
                    </td>{/if}
            </tr>
        {/if}
    {/foreach}
