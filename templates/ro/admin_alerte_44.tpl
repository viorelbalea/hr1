<tr>
    <td>{translate label='Numar zile concediu fara plata'}:</td>
...
<tr>
    <td>{translate label='Tipuri de concediu'}:</td>
    <td>
        {foreach from=$vacationTypes item=item}
            <input type="checkbox" name="settings[vacationTypes][{$item}]" value="1"
                   {if !empty($alerte[$smarty.get.ID].Settings.vacationTypes) && isset($alerte[$smarty.get.ID].Settings.vacationTypes.$item)}checked{/if}>
            {$item}
            <br>
        {/foreach}
    </td>
