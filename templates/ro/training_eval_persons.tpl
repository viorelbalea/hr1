...
<table cellspacing="0" cellpadding="0" width="100%" class="filter" height="40">
    <tr>
        <td style="padding-left: 4px;" width="75" nowrap="nowrap">{translate label='Cauta dupa:'}</td>
        <td style="padding-left: 2px;" width="70">
            <select id="DistrictID" name="DistrictID"
                    onchange="if (this.value>0) window.location.href = './?m=training&o=evalPersons&DivisionID=' + document.getElementById('DivisionID').value + '&DistrictID=' + this.value"
                    class="cod">
        ...
        {if !empty($divisions)}
            <td style="padding-left: 2px;" width="75">
                <select id="DivisionID" name="DivisionID"
                        onchange="window.location.href = './?m=training&o=evalPersons&DistrictID=' + document.getElementById('DistrictID').value + '&DivisionID=' + this.value"
                        class="dropdown">
    ...
    <tr>
        ...
        <td style="padding-left: 50px;">
            {translate label='Afiseaza doar angajatii'}: <input type="checkbox"
                                                                onclick="if (this.checked) window.location.href = './?m=training&o=evalPersons&Status=2'; else window.location.href = './?m=training&o=evalPersons';"
    ...
    {foreach from=$persons key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a href="./?m=training&o=forms&PersonID={$item.PersonID}" class="blue"><b>{$item.FullName}</b></a></td>
                {if !empty($personalisedlist.Eval)}
