...
<table width="100%" cellspacing="0" cellpadding="2" border="1">
    <tr>
        ...
        <td rowspan="2"><b>{translate label='Status'}</b></td>
        <td rowspan="2"><b>{translate label='Tip'}</b></td>
        <td rowspan="2">
        ...
        <td colspan="2"><b>{translate label='Istoric'}</b></td>
    </tr>
    <tr>
        <td>{translate label='Status'}</td>
        <td>{translate label='Descriere'}</td>
    </tr>
    ...
    {if count($tickets) > 0}
        {foreach from=$tickets key=key item=item}
            <tr>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$key|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.CompanyName|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.ContactName|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.StatusName|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.TypeName|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.FullName|default:'&nbsp;'}</td>
            <td rowspan="{if $item.HistoryCount > 0}{$item.HistoryCount}{else}1{/if}">{$item.CreateDate|default:'&nbsp;'}</td>
            {if $item.HistoryCount > 0}
                {foreach from=$item.History item=history name=hist}
                    {if $smarty.foreach.hist.iteration > 1}
                        </tr>
                        <tr>
                    {/if}
                    <td>{$history.StatusName|default:'&nbsp;'}</td>
                    <td style="max-width: 200px;">{$history.Comment|default:'&nbsp;'}</td>
                {/foreach}
            {else}
                <td colspan="2">&nbsp;</td>
            {/if}
