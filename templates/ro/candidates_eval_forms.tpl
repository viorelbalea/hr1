...
<div class="filter">
    ...
    <label><A HREF="#" onClick="cal1.select(document.getElementById('StartDate'),'anchor1','yyyy-MM-dd'); return false;" NAME="anchor1" ID="anchor1"><img src="./images/cal.png"
                                                                                                                                                          border="0"></A></label>
    <label>pana la data de sfarsit</label> <input type="text" name="EndDate" id="EndDate" class="formstyle" value="{$smarty.get.EndDate|default:''|date_format:"%Y-%m-%d"}"
    ...
    {if $smarty.get.o=='forms-evaluator'}
            window.location.href = './?m=candidates-eval&o=forms-evaluator&PersonID=' + document.getElementById('PersonID').value + '&StartDate=' + document.getElementById('StartDate').value + '&EndDate=' + document.getElementById('EndDate').value + '&res_per_page=' + document.getElementById('res_per_page').value
    {else}
            window.location.href = './?m=candidates-eval&o=forms&PersonID=' + document.getElementById('PersonID').value + '&StartDate=' + document.getElementById('StartDate').value + '&EndDate=' + document.getElementById('EndDate').value + '&res_per_page=' + document.getElementById('res_per_page').value
    {/if}
            ">
...
<table cellspacing="0" cellpadding="2" width="100%" class="grid" style="margin-top:6px;">
    <tr>
        ...
        <td class="bkdTitleMenu">{orderby label='Persoana evaluata' request_uri=$request_uri order_by=EvaluatedName}</td>
        <td class="bkdTitleMenu">{orderby label='Relatie' request_uri=$request_uri order_by=RelationID}</td>
    ...
    {foreach from=$evalForms key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST">
                    {if $item.Evaluated}
                        <table class="grid" cellspacing="0" cellpadding="2" width="100%">
                            <tr>
                                <td class="bkdTitleMenu">{translate label='Formular'}</td>
                                <td class="bkdTitleMenu">{translate label='Evaluator'}</td>
                                <td class="bkdTitleMenu">{translate label='Status'}</td>
                                <td class="bkdTitleMenu">{translate label='Calificativ'}</td>
                            </tr>
                            {foreach from=$item.Evaluated item=c name=iter2}
                                <tr>
                                    <td width="130" class="celulaMenuST"><a
                                                href="./?m=candidates-eval&o=eval&EvalFormID={$c.EvalFormID}{if $c.EvaluatorExternEmail}&hashkey={$c.EvaluatorExternEmail|md5}{/if}"
                                                class="blue"><b>deschide</b></a></td>
                                    <td width="130" class="celulaMenuST">{if $c.FullName}{$c.FullName|default:'-'}{else}{$c.EvaluatorExternEmail}{/if}</td>
                ...
                <td class="celulaMenuST">{if empty($item.isPersonExtern)}{$item.EvaluatedName|default:'-'}{else}{$item.PersonExternEmail}{/if}</td>
