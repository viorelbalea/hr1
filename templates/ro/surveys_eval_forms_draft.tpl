<div class="submeniu">
    ...
    {if $smarty.session.USER_ID == 1 || $smarty.session.ACCESSSURVEY == 1 || $smarty.session.ACCESSSURVEY == 3}
        <a href="./?m=surveys&o=formsDraft" class="selected">{translate label='Formulare studii'}</a>
...
<table cellspacing="0" cellpadding="2" width="100%" class="grid" style="margin-top:6px;">
        ...
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a
                            href="./?m=surveys&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&action=edit"
                ...
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSSURVEY == 2 || $smarty.session.ACCESSSURVEY == 3}
                    <td class="celulaMenuST"><a class="blue" href="#"
                                                onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa creezi un duplicat al acestui formular?'}'))window.location.href='./?m=surveys&o=evalDraft&EvalFormDraftID={$key}&action=clone'; return false;">Cloneaza
                        formular</a></td>{/if}
                {if $smarty.session.USER_ID==1 || $smarty.session.ACCESSSURVEY == 2 || $smarty.session.ACCESSSURVEY == 3}
                    <td class="celulaMenuSTDR" style="text-align: center;"><a href="#"
                                                                              onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest formular?'}')) window.location.href='./?m=surveys&o=evalDraft&EvalFormDraftID={$key}&action=delete'; return false;">sterge</a>
                    </td>{/if}
            </tr>
        {/if}
    {/foreach}
    ...
    {if count($evalFormsDraft)<1}
        <tr height="30">
            <td colspan="100" class="celulaMenuSTDR">{translate label='Nici un formular de studiu!'}</td>
        </tr>
    {/if}
    <tr>
