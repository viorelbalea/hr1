            ...
            <form action="{$smarty.server.REQUEST_URI}" method="post" name="pers" onsubmit="return validateForm(this);">
                <fieldset>
                    ...
                    <table border="0" cellpadding="4" cellspacing="0" class="screen">
                        ...
                        <tr>
                            <td><b>{translate label='Arma'}:</b></td>
                        ...
                        {if $info.rw == 1 || !empty($smarty.post)}
                            <tr>
                                <td>&nbsp;</td>
                                <td><input type="submit" value="{translate label='Salveaza'}" class="formstyle"> <input type="button" value="{translate label='Anuleaza'}"
                        ...
                        <tr>
                            ...
                            <td>{if $info.rw == 1}
                                    <div id="button_mod"><a href="#"
                                                            onclick="if (document.getElementById('Emitent_{$key}').value && document.getElementById('Serie_{$key}').value && document.getElementById('No_{$key}').value && document.getElementById('StartDate_{$key}').value && checkDate(document.getElementById('StartDate_{$key}').value, 'Data inceput') && document.getElementById('StopDate_{$key}').value && checkDate(document.getElementById('StopDate_{$key}').value, 'Data sfarsit')) window.location.href = './?m=persons&o=military&PersonID={$smarty.get.PersonID}&action=edit&PermisID={$key}&Emitent=' + escape(document.getElementById('Emitent_{$key}').value) + '&Serie=' + escape(document.getElementById('Serie_{$key}').value) + '&No=' + escape(document.getElementById('No_{$key}').value) + '&StartDate=' + document.getElementById('StartDate_{$key}').value + '&StopDate=' + document.getElementById('StopDate_{$key}').value; else alert('{translate label='Completati Emitent, Serie, Numar, Data de inceput, Data de sfarsit!'}'); return false;"
                                                            title="{translate label='Modifica permis'}"><b>Mod</b></a></div>{/if}</td>
                            <td>{if $info.rw == 1}
                                    <div id="button_del"><a href="#"
                                                            onclick="if (confirm('{translate label='Sunteti sigur(a)?'}')) window.location.href = './?m=persons&o=military&PersonID={$smarty.get.PersonID}&action=del&PermisID={$key}'; return false;"
                    ...
                    <tr>
                        ...
                        <td colspan="2">{if $info.rw == 1}
                                <div id="button_add"><a href="#"
                                                        onclick="if (document.getElementById('Emitent_0').value && document.getElementById('Serie_0').value && document.getElementById('No_0').value && document.getElementById('StartDate_0').value && checkDate(document.getElementById('StartDate_0').value, 'Data inceput') && document.getElementById('StopDate_0').value && checkDate(document.getElementById('StopDate_0').value, 'Data sfarsit')) window.location.href = './?m=persons&o=military&PersonID={$smarty.get.PersonID}&action=new&Emitent=' + escape(document.getElementById('Emitent_0').value) + '&Serie=' + escape(document.getElementById('Serie_0').value) + '&No=' + escape(document.getElementById('No_0').value) + '&StartDate=' + document.getElementById('StartDate_0').value + '&StopDate=' + document.getElementById('StopDate_0').value; else alert('{translate label='Completati Emitent, Serie, Numar, Data de inceput, Data de sfarsit!'}'); return false;"
