...
<table cellspacing="0" cellpadding="0" width="100%" border="0" class="filter">
    <tr height="40">
        ...
        <td width="60">
            <select id="search_for" nume="search_for" class="cod">
                ...
                <option value="name" {if $smarty.get.search_for == 'name'}selected{/if}>{translate label='Nume document'}</option>
                <option value="descr" {if $smarty.get.search_for == 'descr'}selected{/if}>{translate label='Descriere document'}</option>
        ...
        <td style="padding-left: 4px;" width="60"><input type="button" value="Cauta" class="cod"
                                                         onclick="window.location.href = './?m=persons&o=docs&PersonID={$smarty.get.PersonID}&search_for=' + document.getElementById('search_for').value + '&keyword=' + escape(document.getElementById('keyword').value) + '&res_per_page=' + document.getElementById('res_per_page').value">
        </td>
        <td>&nbsp;</td>
    ...
    <tr height="40">
        <td colspan="6" style="padding-left: 4px;"><input type="button" value="Adauga document"
                                                          onclick="window.location.href = './?m=persons&o=docs&PersonID={$smarty.get.PersonID}&action=new';"></td>
    </tr>
</table>
</br>
<table cellspacing="0" cellpadding="2" width="100%">
    <tr>
        ...
        <td class="bkdTitleMenu"><span class="TitleBox">{orderby label='Nume document' request_uri=$request_uri order_by=DocName}</td>
        <td class="bkdTitleMenu"><span class="TitleBox">{orderby label='Cod document' request_uri=$request_uri order_by=DocCode}</td>
        <td class="bkdTitleMenu"><span class="TitleBox">{translate label='Descriere'}</span></td>
        <td class="bkdTitleMenu"><span class="TitleBox">{orderby label='Data' request_uri=$request_uri order_by=a.CreateDate}</td>
        ...
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST">
                    ...
                    <a href="{$item.curr_filename}" title="{translate label='Acceseaza fisierul'} {$item.FileName}" target="_blank" class="target1"><b>{$item.DocName}</b></a>
                ...
                <div id="button_mod"><a href="./?m=persons&o=docs&PersonID={$smarty.get.PersonID}&action=edit&DocID={$key}" title="{translate label='Modifica'}"><b>Mod</b></a>
                </div>{else}&nbsp;{/if}</td>
                {if $item.rw==1}
                    <td class="celulaMenuSTDR" style="text-align: center;">
                    <div id="button_del"><a href="#"
                                            onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest document?'}')) window.location.href='./?m=persons&o=docs&PersonID={$smarty.get.PersonID}&action=del&DocID={$key}'; return false;"
...
<script type="text/javascript" charset="utf-8">
    $(document).ready(function () {
        {/literal}
        {if ($smarty.get.alertemailsent == 1)}
        ...
        $('.target1').contextMenu('context-menu-1', {
            ...
            'Trimite linkul pe mail': {
                click: function (element) {
                    document.location = 'senddocumentbymail.php?doc=' + element.attr("href") + '&mail=' + prompt('Catre ce email doriti trimiterea') + '&urlreturn=' + encodeURIComponent('?m=persons&o=docs&PersonID={/literal}{$smarty.get.PersonID}{literal}');
                },
