...
<div class="filter">
    ...
    <select id="DistrictID" name="DistrictID"
            onchange="if (this.value>0) window.location.href = './?m=performance&o=objective&DivisionID=' + document.getElementById('DivisionID').value + '&DistrictID=' + this.value"
            class="cod">
    ...
    <select id="search_for" nume="search_for" class="cod">
        ...
        <option value="CNP" {if $smarty.get.search_for == 'CNP'}selected{/if}>CNP</option>
    </select>
    ...
    <input type="button" value="Cauta" class="cod"
           onclick="window.location.href = './?m=performance&o=objective&DistrictID=' + document.getElementById('DistrictID').value + '&CityID=' + document.getElementById('CityID').value + '&Status=' + document.getElementById('Status').value + '&CompanyID=' + document.getElementById('CompanyID').value + '&DivisionID=' + document.getElementById('DivisionID').value + '&DimensionID=' + document.getElementById('DimensionID').value + '&DepartmentID=' + document.getElementById('DepartmentID').value + '&CostCenterID=' + document.getElementById('CostCenterID').value + '&Sex=' + document.getElementById('Sex').value + '&search_for=' + document.getElementById('search_for').value + '&keyword=' + escape(document.getElementById('keyword').value) + '&res_per_page=' + document.getElementById('res_per_page').value">
    ...
    {if !empty($divisions)}
        <select id="DivisionID" name="DivisionID"
                onchange="window.location.href = './?m=performance&o=objective&DistrictID=' + document.getElementById('DistrictID').value + '&DivisionID=' + this.value"
                class="dropdown">
    ...
    <td style="padding-left: 10px;">
        ...
        <input type="checkbox" onclick="if (this.checked) window.location.href = '{$request_uri}&Status='; else window.location.href = '{$request_uri}&Status=all';"
               {if empty($smarty.get.Status)}checked{/if}>
        ...
        <label>{translate label='inregistrari '}</label><br/>
    ...
    {foreach from=$persons key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <td class="celulaMenuST"><a href="./?m=performance&o=goals&PersonID={$item.PersonID}" class="blue"><b>{$item.LastName}</b></a></td>
                <td class="celulaMenuST"><a href="./?m=performance&o=goals&PersonID={$item.PersonID}" class="blue"><b>{$item.FirstName}</b></a></td>
                {if !empty($personalisedlist.Performance)}
                ...
                {else}
                    ...
                    <td class="celulaMenuSTDR">{$item.Mobil|default:'&nbsp;'}</td>
                {/if}
            </tr>
        {/if}
    {/foreach}
