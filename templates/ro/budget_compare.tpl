...
<div class="filter">
    ...
    <select id="month">
        ...
        {foreach from=$months key=mon item=month}
            <option value="{$mon}" {if $mon == $smarty.get.month}selected{/if}>{$month}</option>
        {/foreach}
    </select>
    ...
    <input type="button"
           onclick="if (document.getElementById('year').value > 0 && document.getElementById('month').value > 0) window.location.href = './?m=budgets&o=compare&year=' + document.getElementById('year').value + '&month=' + document.getElementById('month').value + '&CompanyID=' + document.getElementById('CompanyID').value; else alert('{translate label='Alegeti anul si luna pentru comparare!'}');"
           value="Compara">
    <br/>
