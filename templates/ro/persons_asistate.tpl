...
<table border="0" cellpadding="4" cellspacing="0" class="screen" width="100%">
    ...
    <tr>
        <td colspan="2" class="celulaMenuSTDR" style="vertical-align: top; padding-left: 10px; padding-right: 10px;">
            <br>
            <fieldset>
                <legend>{translate label='Persoane asistate/plasament'}</legend>
                <table border="0" cellpadding="4" cellspacing="0">
                    <tr>
                        <td>{translate label='Nume si prenume'}</td>
                        {*                        <td>{translate label='Calitate'}</td>*}
                        <td>{translate label='CNP'}</td>
                        <td>{translate label='Data nasterii'}</td>
                        <td>{translate label='Tip handicap'}</td>
                        <td>{translate label='Certificat incadrare'}</td>
                        <td>{translate label='Data start'}</td>
                        ...
                        {if $key > 0}
                            <tr>
                                <form action="{$smarty.server.REQUEST_URI}&action=edit_pers&ID={$key}" method="post" name="pers_{$key}">
                                    ...
                                    <td>
                                        <input type="text" id="BirthDate" name="BirthDate" class="formstyle" value="{$item.BirthDate}" size="10" maxlength="10">
                                        <SCRIPT LANGUAGE="JavaScript" ID="js0_{$key}">
                                            var cal0_{$key} = new CalendarPopup();
                                            cal0_{$key}.isShowNavigationDropdowns = true;
                                            cal0_{$key}.setYearSelectStartOffset(10);
                                            //writeSource("js0_{$key}");
                                        </SCRIPT>
                                        <A HREF="#" onClick="cal0_{$key}.select(document.pers_{$key}.BirthDate,'anchor0_{$key}','dd.MM.yyyy'); return false;" NAME="anchor0_{$key}"
                                           ID="anchor0_{$key}"><img src="./images/cal.png" border="0"></A>
                                    </td>
                                    <td><input type="text" name="TipHandicap" value="{$item.TipHandicap}" size="30"></td>
                                    <td><input type="text" name="Certificat" value="{$item.Certificat}" size="30"></td>
                                    <td>
                                    ...
                                    {if $persons.0.rw==1}
                                        <td>
                                            <div id="button_mod"><a href="#"
                                                                    onclick="if (!is_empty(document.pers_{$key}.Nume.value) && !is_empty(document.pers_{$key}.CNP.value) && document.pers_{$key}.BirthDate.value && checkDate(document.pers_{$key}.BirthDate.value, 'Data nasterii') && document.pers_{$key}.StartDate.value && checkDate(document.pers_{$key}.StartDate.value, 'Data initiala')) document.pers_{$key}.submit(); else alert('{translate label='Completati Nume si prenume, CNP, Data nasterii ! '}'); return false;"
