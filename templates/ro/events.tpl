...
<div class="filter">
    ...
    <select id="EventType" name="EventType" class="cod">
        <option value="0">{translate label='Tip intalnire'}</option>
            ...
            <tr height="30">
                ...
                {if $smarty.session.USER_ID==1}
                    <td class="celulaMenuSTDR" style="text-align: center;"><a href="#"
                                                                              onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest eveniment?'}')) window.location.href='./?m=events&o=del&EventID={$key}'; return false;">{translate label='sterge'}</a>
                    </td>{/if}
            </tr>
        {/if}
    {/foreach}
