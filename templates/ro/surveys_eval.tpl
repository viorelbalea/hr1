	...
	-->
        ...
        <tr>
            <td colspan="2" class="celulaMenuSTDR" style="padding: 10px;">
                <fieldset>
                    <legend>Studiu <b>{$person.FullName}</b> pentru perioada <b>{$StartDate}</b> - <b>{$EndDate}</b>
                    </legend>
                    <table cellspacing="0" cellpadding="4">
                        <tr valign="top">
                            ...
                            {if ($isPerson && $Completed) || $isEvaluator  || $smarty.session.USER_ID==1}
                                <td align="center" class="bkdTitleMenu" width="150">
                                    <b>{translate label='Nota studiu'}</b></td>
                        ...
                        {foreach from=$eval key=k item=section}
                            <tr>
                                <td colspan="50" style="border-bottom:1px solid #EDEDED; background-color:#FCFCFC;">
                                    <strong><br/>{$k}</strong></td>
                            </tr>
                        ...
                        {if ($isEvaluator && $Completed==0) || $smarty.session.USER_ID==1}
                            <tr>
                                <td>{translate label=' Studiul este incheiat?'}
                                    <br/>{translate label='(se bifeaza de catre persoana evaluatoare)'}</td>
                        ...
                        <tr>
                            <td colspan="3">
                                {if ($smarty.session.PersonID==$evalEvaluatorID && $Completed==0) && ($smarty.session.ACCESSSURVEY == 2 || $smarty.session.ACCESSSURVEY == 3)}
