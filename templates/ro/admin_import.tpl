...
<form action="./?m=admin&o=import" method="post" enctype="multipart/form-data" name="upload_pers">
        ...
        <table cellspacing="0" cellpadding="2" width="100%" class="grid">
            ...
            {foreach from=$persons key=key item=item name=iter1}
                <tr height="30">
                    ...
                    {if $personsHeader.4!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][4]" value="{$item.4}"/></td>{/if}
                    {if $personsHeader.5!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][5]" value="{$item.5}"/></td>{/if}
                    {if $personsHeader.6!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][6]" value="{$item.6}"/></td>{/if}
                    {if $personsHeader.7!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][7]" value="{$item.7}"/></td>{/if}
                    {if $personsHeader.8!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][8]" value="{$item.8}"/></td>{/if}
                    {if $personsHeader.9!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][9]" value="{$item.9}"/></td>{/if}
                    {if $personsHeader.10!=''}
                        <td class="celulaMenuST"><input type="text" name="person[{$key}][10]" value="{$item.10}"/></td>{/if}
                ...
                <tr height="30">
                    <td style="text-align:center;" colspan="100" class="celulaMenuSTDR">
                        <b>{translate label='Va rugam incarcati un fisier format .xls impreuna cu capetele de tabel.'} <br/>
                            <br/>{translate label=' Coloanele trebuie sa fie in ordine: Marca/ Nume,prenume/ Telefon/ E-mail/ CNP/ Ore pe zi/ CI serie/ CI Numar/ CI Emitent'}
