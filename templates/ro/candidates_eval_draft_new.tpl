    ...
    <tr>
        <td class="celulaMenuST" style="vertical-align: top; padding-left: 10px; padding-bottom: 10px; padding-right: 10px;">
            ...
            <form action="{$request_uri}" method="post">
                {else}
                <form action="{$request_uri}&o=evalDraft&action=new" method="post">
                        ...
                        <table border="0" cellpadding="4" cellspacing="0" class="screen" width="600">
                            ...
                            {foreach from=$sections item=item}
                                <tr>
                                    ...
                                    <td>
                                        <div id="button_mod"><a href="#"
                                                                onclick="if (!is_empty(document.getElementById('Name_{$item.SectionID}').value) && !is_empty(document.getElementById('Pondere_{$item.SectionID}').value) && !is_empty(document.getElementById('Priority_{$item.SectionID}').value)) window.location.href = './?m=candidates-eval&o=evalDraft&EvalFormDraftID={$smarty.get.EvalFormDraftID}&action=edit_section&SectionID={$item.SectionID}&Name=' + document.getElementById('Name_{$item.SectionID}').value + '&Pondere=' + document.getElementById('Pondere_{$item.SectionID}').value +'&Priority=' + document.getElementById('Priority_{$item.SectionID}').value + '&Status=' + (document.getElementById('Status_{$item.SectionID}').checked ? 1 : 0); else alert('{translate label='Nu ati specificat toate informatiile despre sectiune!'}'); return false;"
                                                                title="Modifica sectiune"><b>Mod</b></a></div>
                                    </td>
                                    <td>
                                        <div id="button_del"><a href="#"
                                                                onclick="if (confirm('{translate label='Sunteti sigur(a)?'}')) window.location.href = './?m=candidates-eval&o=evalDraft&EvalFormDraftID={$smarty.get.EvalFormDraftID}&action=del_section&SectionID={$item.SectionID}'; return false;"
                                ...
                                {foreach from=$item.Questions item=item2 key=key2 name=name2}
                                    <form action="?m=candidates-eval&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&action=edit_question&EvalQuestionID={$item2.EvalQuestionID}"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div id="button_del"><a href="#"
                                                                        onclick="if (confirm('Sunteti sigura(a)?')) window.location.href = './?m=candidates-eval&o=evalDraft&EvalFormDraftID={$item.EvalFormDraftID}&EvalQuestionID={$item2.EvalQuestionID}&action=del_question'; return false;"
                                ...
                                <form action="?m=candidates-eval&o=evalDraft&EvalFormDraftID={$smarty.get.EvalFormDraftID}&action=edit_question&EvalQuestionID=0" method="post"
                        ...
                        {if $key>0}
                            <tr height="30">
                                ...
                                <td class="celulaMenuSTDR"><a class="blue"
                                                              href="./?m=candidates-eval&o=forms&PersonID={$item.PersonID}&EvalFormDraftID={$smarty.get.EvalFormDraftID}">{translate label='Vezi lista evaluari '}</a>
                                </td>
                            </tr>
                        {/if}
                        {foreachelse}
