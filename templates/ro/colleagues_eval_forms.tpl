<div class="submeniu">
    <a href="./?m=colleagues-eval&o=forms" class="selected">{translate label='Lista evaluari'}</a>
...
<div class="filter">
    ...
    {if $smarty.get.o=='forms-evaluator'}
            window.location.href = './?m=colleagues-eval&o=forms-evaluator&PersonID=' + document.getElementById('PersonID').value + '&StartDate=' + document.getElementById('StartDate').value + '&EndDate=' + document.getElementById('EndDate').value + '&res_per_page=' + document.getElementById('res_per_page').value
    {else}
            window.location.href = './?m=colleagues-eval&o=forms&PersonID=' + document.getElementById('PersonID').value + '&StartDate=' + document.getElementById('StartDate').value + '&EndDate=' + document.getElementById('EndDate').value + '&res_per_page=' + document.getElementById('res_per_page').value
    {/if}
            ">
                ...
                <td class="celulaMenuST">
                    {if $item.Evaluated}
                        <table class="grid" cellspacing="0" cellpadding="2" width="100%">
                            <tr>
                                <td class="bkdTitleMenu"><span class="TitleBox">{translate label='Formular'}</span></td>
                                <td class="bkdTitleMenu"><span class="TitleBox">{translate label='Evaluator'}</span>
                                </td>
                                <td class="bkdTitleMenu"><span class="TitleBox">{translate label='Status'}</span></td>
                                <td class="bkdTitleMenu"><span class="TitleBox">{translate label='Calificativ'}</span>
                                </td>
                            </tr>
                            {foreach from=$item.Evaluated item=c name=iter2}
                                <tr>
                                    <td width="130" class="celulaMenuST"><a
                                                href="./?m=colleagues-eval&o=eval&EvalFormID={$c.EvalFormID}"
                                                class="blue"><b>deschide</b></a></td>
                                    <td width="130" class="celulaMenuST">{$c.FullName|default:'-'}</td>
                                    <td width="100"
                                        class="celulaMenuST">{if $c.Completed==1}{translate label='Incheiata'}{else}{translate label='Neincheiata'}{/if}</td>
                                    <td width="220"
