...
<div class="filter">
    ...
    <input type="text" name="StartDate" id="StartDate" class="formstyle" value="{$start_date}" size="10" maxlength="10" readonly>
    ...
    <label>si</label>
    <input type="text" name="EndDate" id="EndDate" class="formstyle" value="{$end_date}" size="10" maxlength="10" readonly>
...
{if !empty($smarty.get.StartDate) && !empty($smarty.get.EndDate)}
    ...
    <form action="{$smarty.server.REQUEST_URI}" method="post">
        <table width="100%" cellspacing="0" cellpadding="4" class="grid">
            <tr valign="bottom">
                ...
                {foreach from=$cal key=data item=wday name=iter}
                    <td class="celulaMenuST" style="text-align:center"
                        {if $wday=='D' || $wday=='S' || isset($legal.$data)}bgcolor="#fcde63"{else}{math equation="x+1" x=$ZL assign="ZL"}{/if}>
                        <b>{$smarty.foreach.iter.iteration}</b></td>
                {/foreach}
                {if $smarty.session.USER_ID == 1 || $smarty.session.USER_RIGHTS3.11.5.1 == 2}
                    <td class="celulaMenuST" style="text-align:center"><input type="submit" name="valid" value="{translate label='Validare'}"></td>
                {/if}
            </tr>
            <tr>
                ...
                {foreach from=$cal key=data item=wday}
                    <td class="celulaMenuST" style="text-align:center" {if $wday=='D' || $wday=='S' || isset($legal.$data)}bgcolor="#fcde63"{/if}><b>{$wday}</b></td>
                {/foreach}
                {if $smarty.session.USER_ID == 1 || $smarty.session.USER_RIGHTS3.11.5.1 == 2}
            ...
            {if $persid > 0}
            <tr bgcolor="#ffffff">
                <td class="celulaMenuST">{$smarty.foreach.iter.iteration-1}</td>
                <td class="celulaMenuST"><a href="#" onclick="return false; getStat({$persid}, '{$smarty.get.StartDate}', '{$smarty.get.EndDate}'); return false;"
                                            title="{translate label='raport pontaj'}">{$item.FullName}</a></td>
                {foreach from=$cal key=data item=wday}
                    <td class="celulaMenuST" style="text-align:center" {if $wday=='D' || $wday=='S' || isset($legal.$data)}bgcolor="#fcde63"{/if}>
                ...
                {/foreach}
                {if $smarty.session.USER_ID == 1 || $smarty.session.USER_RIGHTS3.11.5.1 == 2}
                    <td class="celulaMenuST" style="text-align:center"><input type="checkbox" id="pvalid_{$persid}" name="pvalid[{$persid}]" value="1"></td>
                {/if}
            <tr>
                {/if}
                {/foreach}
