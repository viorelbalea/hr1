...
<table cellspacing="0" cellpadding="0" width="100%" border="0" class="filter">
    <tr height="40">
        ...
        <td style="padding-left: 4px;" width="60"><input type="button" value="Cauta" class="cod"
                                                         onclick="window.location.href = './?m=cars&o=docs&CarID={$smarty.get.CarID}&search_for=' + document.getElementById('search_for').value + '&keyword=' + escape(document.getElementById('keyword').value) + '&res_per_page=' + document.getElementById('res_per_page').value">
        </td>
        <td>&nbsp;</td>
    ...
    <tr height="40">
        <td colspan="6" style="padding-left: 4px;"><input type="button" value="Adauga document"
                                                          onclick="window.location.href = './?m=cars&o=docs&CarID={$smarty.get.CarID}&action=new';"></td>
    </tr>
</table>
</br>
<table cellspacing="0" cellpadding="2" width="100%">
    <tr>
        ...
        <td class="bkdTitleMenu">{orderby label="Nume document" request_uri=$request_uri order_by=DocName}</td>
    ...
    {foreach from=$docs key=key item=item name=iter1}
        {if $key>0}
            <tr height="30">
                ...
                <div id="button_mod"><a href="./?m=cars&o=docs&CarID={$smarty.get.CarID}&action=edit&DocID={$key}" title="{translate label='Modifica'}"><b>Mod</b></a>
                </div>{else}&nbsp;{/if}</td>
                {if $item.rw==1}
                    <td class="celulaMenuSTDR" style="text-align: center;">
                    <div id="button_del"><a href="#"
                                            onclick="if (confirm('{translate label='Esti sigur(a) ca vrei sa stergi acest document?'}')) window.location.href='./?m=cars&o=docs&CarID={$smarty.get.CarID}&action=del&DocID={$key}'; return false;"
