...
$start_year = "{$_GET['year']}-01-01";
$end_year = "{$_GET['year']}-12-31";
...
$query = "SELECT a.PersonID, a.WorkTime, a.WorkTimeAt, b.StartDate, b.StopDate, b.ContractExpDate, b.CompanyID, b.ContractType, b.SuspendDate, b.ReturnDate
          FROM   persons a
	         INNER JOIN payroll b ON a.PersonID = b.PersonID AND b.StartDate <= '{$end_year}'
          WHERE  a.Status > 0
	  ORDER  BY a.PersonID";
$conn->query($query);
...
foreach ($persons as $PersonID => $person) {
    ...
    if (!empty($company_settings[$person['CompanyID']]['vacation']['accepted_seniority']) && $company_settings[$person['CompanyID']]['vacation']['accepted_seniority'] == 2) {
        ...
        if (!empty($worktimetest)) {
            ...
            $months = $arr[1];
            $years = $arr[0];
        } else { // Add from CIM History
            $conn->query("SELECT DataIni AS StartDate, DataFin AS EndDate FROM persons_cm WHERE PersonID = $PersonID AND DataIni < '{$start_year}'");
            while ($row = $conn->fetch_array()) {
    ...
    foreach ((array)$seniority[$person['CompanyID']] as $max_seniority => $v) {
        if ($persons[$PersonID]['years'] < $max_seniority) {
            ...
            $persons[$PersonID]['rep_date_limit'] = date('Y-m-d', strtotime("{$_GET['year']}-{$v['rep_month_limit']}-{$v['rep_day_limit']}"));
            $persons[$PersonID]['rep_date_limit_past'] = date('Y-m-d', strtotime($_GET['year'] - 1 . "-{$v['rep_month_limit']}-{$v['rep_day_limit']}"));
    ...
    if (substr($person['StartDate'], 0, 4) == $_GET['year']) {
    ...
    $conn->query("SELECT TotalCO, Invoire, TotalCORef, 
			 (SELECT SUM(DaysNo) FROM vacations_details WHERE PersonID = a.PersonID AND Year = '" . ($_GET['year'] - 1) . "' AND Type = 'CO' AND Aprove >= 0) AS EffCO
                  FROM   vacations a
		  WHERE  PersonID = $PersonID AND Year = '" . ($_GET['year'] - 1) . "' ");
    $LeftCO = 0;
    $LeftCOINV = 0;
    $bDaysLost = 0;
    ...
    //cip
    $query = "SELECT DaysNo from vacations_details
			WHERE
				PersonID= $PersonID AND
				Year = '" . ($_GET['year'] - 1) . "' AND
				Aprove >=0 AND
				Details ='Concediu de studiu fara plata'
				";
    $conn->query($query);
    $FaraPlata = 0;
    while ($row = $conn->fetch_array()) {
        $FaraPlata += (int)$row['DaysNo'];
    }
    // Substract days id Vacation is without pay in the selected Year
    $persons[$PersonID]['TotalCO'] -= $FaraPlata;
    //eocip
    ...
    if ($bDaysLost == 1 && $LeftCOINV > 0) {
        ...
        $query = "SELECT StartDate, StopDate, DaysNo 
					...
					Type = 'CO' AND 
					Aprove >= 0 AND 
					StartDate <= '{$persons[$PersonID]['rep_date_limit_past']}' ";
        $conn->query($query);
        while ($row = $conn->fetch_array()) {
    ...
    $conn->query("SELECT VacationID, Closed, VacRecalc FROM vacations WHERE PersonID = $PersonID AND Year = '" . $_GET['year'] . "'");
    if ($row = $conn->fetch_array()) {
        ...
        $conn->query("INSERT INTO vacations(UserID, PersonID, Year, TotalCO, TotalCORef, CreateDate, ReportDateLimit) VALUES(1, $PersonID, '" . $_GET['year'] . "', '{$persons[$PersonID]['TotalCO']}', '{$persons[$PersonID]['TotalCORef']}', CURRENT_TIMESTAMP, '{$persons[$PersonID]['rep_date_limit']}')");
    }
}
