...
switch ($o) {
    ...
    ##########################################################################################################################
    case 'formsDraft':
        $evalFormsDraft = Survey::getEvalFormsDraft();
        if (!empty($_GET['action'])) {
            switch ($_GET['action']) {
                ...
                case 'print_all':
                    ...
                    $smarty->display('surveys_eval_forms_draft_print.tpl');
                    exit;
                    break;
                case 'export_doc':
            ...
            $request_uri = !empty($_GET['res_per_page']) ? "./?m=surveys&o=formsDraft&FunctionID={$_GET['FunctionID']}&search_for={$_GET['search_for']}&keyword={$_GET['keyword']}&res_per_page={$_GET['res_per_page']}" : "./?m=surveys&o=formsDraft";
            $smarty->assign(array('evalFormsDraft' => $evalFormsDraft,
                'evalPersonsFormsDraft' => Survey::getAllPersonsByFormsDraft(),
            ...
            $center_file = 'surveys_eval_forms_draft.tpl';
        }
        break;
    ...
    case 'evalAssign':
        if (!empty($_POST)) {
            try {
                $EvalFormID = Survey::setEvalToPerson($_POST);
                header('Location: ./?m=surveys&o=eval&EvalFormID=' . $EvalFormID);
        ...
        $smarty->assign(array(
            'forms' => Survey::getAllEvalFormsDraft(),
            'functions' => Utils::getFunctions(),
            'relations' => Survey::$msSurveyRelation,
            'persons' => Survey::getPersonsByFunction()
        ));
        $center_file = 'surveys_eval_assign.tpl';
    ...
    case 'evalDraft':
        ...
        if ($_GET['EvalQuestionID'] != '' || !empty($_GET['del'])) {
            try {
                Survey::addEvalQuestion($_GET['EvalQuestionID']);
                header('Location: ./?m=surveys&o=evalDraft&EvalFormDraftID=' . $EvalFormDraftID . '&action=edit');
                exit;
            } catch (Exception $exp) {
                $err->setError($exp->getMessage());
            }
        ...
        if (!empty($_GET['action'])) {
            ...
            switch ($_GET['action']) {
                ...
                case 'new':
                    if (!empty($_POST) && $_GET['EvalQuestionID'] == '') {
                        try {
                            $EvalFormDraftID = Survey::addEvalFormDraft();
                            header('Location: ./?m=surveys&o=evalDraft&EvalFormDraftID=' . $EvalFormDraftID . '&action=edit');
                            exit;
                        } catch (Exception $exp) {
                            $err->setError($exp->getMessage());
                    ...
                    $center_file = 'surveys_eval_draft_new.tpl';
                ...
                case 'edit':
                    ...
                    if (!empty($_POST) && $_GET['EvalQuestionID'] == '') {
                        try {
                            Survey::editEvalFormDraft($EvalFormDraftID, $_POST);
                        } catch (Exception $exp) {
                            $err->setError($exp->getMessage());
                        }
                    }
                    $form = Survey::getEvalFormDraft($EvalFormDraftID);
                    $persons = Survey::getPersonsByFunction($form['FunctionID']);

                    $smarty->assign(array(
                        'form' => $form,
                        'evaledPersons' => Survey::getPersonsByFormDraft($EvalFormDraftID),
                        'sections' => Survey::getFormDraftSections($EvalFormDraftID),
                    ));
                    $center_file = 'surveys_eval_draft_new.tpl';
                    break;
                case 'new_section':
                ...
                case 'del_section':
                    Survey::setSection();
                    header('Location: ./?m=surveys&o=evalDraft&EvalFormDraftID=' . $_GET['EvalFormDraftID'] . '&action=edit');
                    exit;
                    break;
                ...
                case 'clone':
                    ...
                    if (!empty($_GET['EvalFormDraftID'])) {
                        $EvalFormDraftID = (int)$_GET['EvalFormDraftID'];
                        $newEvalFormDraftID = Survey::cloneEvalFormDraft($EvalFormDraftID);
                    }
                    header('Location: ./?m=surveys&o=evalDraft&EvalFormDraftID=' . $newEvalFormDraftID . '&action=edit');
                    exit;
                    break;
                ...
                case 'delete':
                    $EvalFormDraftID = (int)$_GET['EvalFormDraftID'];
                    try {
                        Survey::deleteEvalFormDraft($EvalFormDraftID);
                    } catch (Exception $exp) {
                        $err->setError($exp->getMessage());
                    }
                    header('Location: ./?m=surveys&o=formsDraft');
                    exit;
                    break;
            }
        } else {
            ...
            $smarty->assign(array('evalFormsDraft' => $evalFormsDraft,
                'functions' => Utils::getFunctions(),
                'evaledPersons' => Survey::getPersonsByFormDraft($EvalFormDraftID),
                'request_uri' => !empty($_GET['res_per_page']) ? "./?m=surveys&o=formsDraft&FunctionID={$_GET['FunctionID']}&search_for={$_GET['search_for']}&keyword={$_GET['keyword']}&res_per_page={$_GET['res_per_page']}" : "./?m=surveys",
            ));
            $center_file = 'surveys_eval_draft_new.tpl';
        }
        break;
    ...
    case 'forms-evaluator':
        ...
        $evalForms = Survey::getEvalFormsEvaluator((int)$_GET['PersonID']);

        switch ($_GET['action']) {

            case 'delete';
                $EvalFormID = (int)$_GET['EvalFormID'];
                Survey::deleteEvalForm($EvalFormID);
                header('Location: ./?m=surveys&o=forms&PersonID=' . (int)$_GET['PersonID']);
                exit;
                break;

            case 'set_mediation';
                $EvalFormID = (int)$_GET['EvalFormID'];
                Survey::setMediation($EvalFormID);
                header('Location: ./?m=surveys&o=forms&PersonID=' . (int)$_GET['PersonID']);
                exit;
                break;
            case 'export':
            ...
            case 'print_all':
                ...
                $smarty->display('surveys_eval_forms_print.tpl');
                exit;
                break;
            case 'export_doc':
                header("Content-Type: application/vnd.ms-word");
            ...
            default:
                ...
                $center_file = 'surveys_eval_forms.tpl';
                break;
        }
    ...
    default:
        ...
        $evalForms = Survey::getEvalFormsPersonal((int)$_GET['PersonID']);
        ...
        switch ($_GET['action']) {
            ...
            case 'export':
                ...
                foreach ($evalForms as $k => $v) {
                    $excel[$k]['Nume formular'] = $v['FormName'];
                    $excel[$k]['Functie'] = $v['Function'];
                    $excel[$k]['Relatie'] = Survey::$msSurveyRelation[$v['RelationID']];
                    $excel[$k]['Date inceput'] = $v['StartDate'];
                    $excel[$k]['Data sfarsit'] = $v['EndDate'];
    ...
    case 'eval':
        ...
        $evalPersonID = Survey::getPersonByForm($_GET['EvalFormID']);
        $evalEvaluatorID = Survey::getEvaluatorByForm($_GET['EvalFormID']);

        //Utils::pa($_POST);
        if (!empty($_POST)) {
            Survey::editEval($_POST);

            if (!empty($_POST['Completed']))
                Survey::completeEvalForm((int)$_POST['Completed'], (int)$_GET['EvalFormID']);

            header('Location: ./?m=surveys&o=eval&EvalFormID=' . (int)$_GET['EvalFormID']);
        }
        ...
        if ($_GET['action'] == 'export_doc') {
            ...
            $smarty->display('surveys_eval_print.tpl');
            exit;
        }
        $eval = Survey::getEval($_GET['EvalFormID']);
        ...
        $Completed = Survey::getCompletedStatus($_GET['EvalFormID']);
    ...
    case 'evalPersonsEvaluator':
        ...
        if ($_SESSION['USER_ID'] != 1) {
            if ($o == 'evalPersons' || $o == 'default')
                $persons = Survey::getAllPersons($action);

            elseif ($o == 'evalPersonsEvaluator')
                $persons = Survey::getAllPersonsEvaluator($action);
        ...
        switch ($action) {
            ...
            default:
                ...
                $request_uri = "./?m=surveys&o=evalPersons&DistrictID={$_GET['DistrictID']}&CityID={$_GET['CityID']}&Status={$_GET['Status']}&CompanyID={$_GET['CompanyID']}&DivisionID={$_GET['DivisionID']}&CostCenterID={$_GET['CostCenterID']}&search_for={$_GET['search_for']}&keyword={$_GET['keyword']}&res_per_page={$_GET['res_per_page']}";
                $smarty->assign(array(
                    'persons' => $persons,
                    'status' => $status,
                ...
                if ($_SESSION['USER_ID'] != 1) {
                    if ($o == 'evalPersons' || $o == 'default')
                        $center_file = 'surveys_eval_persons.tpl';

                    elseif ($o == 'evalPersonsEvaluator')
                        $center_file = 'surveys_eval_persons_evaluator.tpl';
