...
class Survey extends ConfigData
{
    ...
    public static function getAllPersons($action = '')
    {
        ...
        $query = "SELECT a.*, d.CityName, e.DistrictName, f.DepartmentID, f.FunctionID,
	              ...
	              FROM   persons a
                       ...
                       INNER JOIN users g ON a.UserID = g.UserID
                       INNER JOIN survey_eval_forms i ON a.PersonID=i.PersonID
				  WHERE  (a.RoleID > 0 AND (a.UserID = {$_SESSION['USER_ID']} OR a.PersonID = '{$_SESSION['PERS']}' OR '{$_SESSION['USER_RIGHTS2'][25][1]}' > 1 OR {$_SESSION['USER_ID']} = 1)) $cond
    ...
    public static function cloneEvalFormDraft($EvalFormDraftID)
    {
        ...
        $query = "INSERT INTO survey_eval_forms_draft (UserID,FormName,CreateDate)
       				SELECT UserID,CONCAT_WS('','$txClone',FormName),CURRENT_TIMESTAMP FROM survey_eval_forms_draft
        ...
        $query = "SELECT * FROM survey_eval_sections a
    			WHERE  a.EvalFormDraftID=$EvalFormDraftID";
        $r1 = $conn->query($query);
        while ($row = $conn->fetch_array($r1)) {
            // Insert new sections
            $query2 = "INSERT INTO survey_eval_sections SET
            ...
            $query3 = "SELECT * FROM survey_eval_questions a
	    			WHERE  a.EvalFormDraftID=$EvalFormDraftID
	    			AND a.SectionID='{$row['SectionID']}'";
            $r3 = $conn->query($query3);
            while ($row3 = $conn->fetch_array($r3)) {
                $query4 = "INSERT INTO survey_eval_questions SET
    ...
    public static function getEvalFormsDraft()
    {
        ...
        $query = "SELECT COUNT(*) AS total
				FROM survey_eval_forms_draft a
        ...
        $query = "SELECT a.EvalFormDraftID, a.FormName, a.CreateDate, DATE_FORMAT(a.CreateDate, '%d.%m.%Y') AS CreateDate, (SELECT COUNT(*) FROM survey_eval_forms WHERE EvalFormDraftID=a.EvalFormDraftID) AS AssignedForms 
    			FROM survey_eval_forms_draft a
    			LEFT JOIN eval_forms c ON a.EvalFormDraftID=c.EvalFormDraftID
    			WHERE (a.UserID = {$_SESSION['USER_ID']} OR '{$_SESSION['USER_SETTINGS'][25]}' > 1 OR {$_SESSION['USER_ID']} = 1) $cond
    ...
    public static function getPersonsByFormDraft($EvalFormDraftID)
    {
        ...
        $query = "SELECT COUNT(b.EvalFormID) AS EvalSum,c.PersonID,c.FullName FROM survey_eval_forms_draft a
				INNER JOIN survey_eval_forms b ON a.EvalFormDraftID=b.EvalFormDraftID
    ...
    public static function addEvalQuestion($EvalQuestionID)
    {
        ...
        if ($EvalQuestionID > 0 && $_GET['action'] == 'del_question') {
            $conn->query("DELETE FROM survey_eval_questions
                          WHERE  EvalQuestionID = $EvalQuestionID AND
                                 NOT EXISTS(SELECT EvalQuestionID FROM survey_eval WHERE EvalQuestionID = $EvalQuestionID)");
            if (!$conn->get_affected_rows()) {
                $conn->query("SELECT EvalFormDraftID FROM survey_eval_questions WHERE EvalQuestionID = $EvalQuestionID");
                $row = $conn->fetch_array();
                $EvalFormDraftID = $row['EvalFormDraftID'];
                echo "<body onload=\"alert('Nu se poate sterge aceasta intrebare deoarece este deja utilizata!'); window.location.href = './?m=surveys-eval&o=evalDraft&EvalFormDraftID=" . $EvalFormDraftID . "&action=edit';\"></body>";
                exit;
            }
            ...
            //exit;
            if ($EvalQuestionID > 0) {
                $conn->query("UPDATE survey_eval_questions SET Question = '$Question', Pondere = '$Pondere', SectionID=$SectionID WHERE EvalQuestionID = $EvalQuestionID");
            } else {
                $conn->query("INSERT INTO survey_eval_questions(UserID,EvalFormDraftID, Question, Pondere, SectionID) VALUES('{$_SESSION['USER_ID']}', '{$_GET['EvalFormDraftID']}','$Question', '$Pondere', $SectionID)");
            }
            if ($conn->errno == 1062) {
    ...
    public static function getEvalQuestions($EvalFormDraftID)
    {
        ...
        global $conn;
        $query = "SELECT *,a.Pondere AS Pondere FROM survey_eval_questions a
    			INNER JOIN survey_eval_sections b ON a.SectionID=b.SectionID
    			WHERE  a.EvalFormDraftID=$EvalFormDraftID
    ...
    public static function getEvalFormsPersonal()
    {
        ...
        $query = "SELECT COUNT(*) AS total FROM survey_eval_forms a
        ...
        $query = "SELECT *, d.FullName as EvaluatedName FROM survey_eval_forms a
    			LEFT JOIN functions b ON a.FunctionID=b.FunctionID
        ...
        foreach ($evalForms as $evalFormID => $evalForm) {
            $query = "SELECT * FROM survey_eval_forms a
    			LEFT JOIN persons c ON a.EvaluatorID=c.PersonID
    ...
    public static function completeEvalForm($CStatus, $EvalFormID)
    {
        ...
        $query = "SELECT Mark,q.Pondere AS Pondere,s.Pondere AS SPondere,s.SectionID, Mark*q.Pondere AS Nominator
					FROM survey_eval e
					      LEFT JOIN survey_eval_questions q ON q.EvalQuestionID=e.EvalQuestionID
					      LEFT JOIN survey_eval_sections s ON q.SectionID=s.SectionID
        ...
        //update eval form
        $query = "UPDATE survey_eval_forms SET Completed=$CStatus, SelfWeighted=$weighted, CompletedDate=CURRENT_TIMESTAMP WHERE EvalFormID=$EvalFormID";
        $conn->query($query);
    }
    ...
    public static function deleteEvalForm($EvalFormID)
    {
        ...
        $query = "DELETE FROM survey_eval_forms WHERE EvalFormID=$EvalFormID";
        $conn->query($query);

        $query = "DELETE FROM survey_eval WHERE EvalFormID=$EvalFormID";
        $conn->query($query);
    }
    ...
    public static function getEval($EvalFormID)
    {
        ...
        $query = "SELECT *,d.Name AS SectionName, b.Pondere AS Pondere, d.Pondere AS SPondere FROM survey_eval a
       			LEFT JOIN survey_eval_questions b ON a.EvalQuestionID=b.EvalQuestionID
       			LEFT JOIN survey_eval_forms c ON c.EvalFormID=a.EvalFormID
       			LEFT JOIN survey_eval_sections d ON b.SectionID=d.SectionID
    ...
    public static function getPersonByForm($EvalFormID)
    {
        ...
        $query = "SELECT b.PersonID FROM survey_eval a
       			LEFT JOIN survey_eval_forms b ON b.EvalFormID=a.EvalFormID
    ...
    public static function getEvaluatorByForm($EvalFormID)
    {
        ...
        $query = "SELECT b.EvaluatorID FROM survey_eval a
       			LEFT JOIN survey_eval_forms b ON b.EvalFormID=a.EvalFormID
    			WHERE  b.EvalFormID=$EvalFormID
    			LIMIT 1";
        $conn->query($query);
...
########################    Evaluations Forms Personal  #######################################
    ...
    public static function getCompletedStatus($EvalFormID)
    {
        ...
        $conn->query("SELECT Completed FROM survey_eval_forms WHERE EvalFormID=$EvalFormID");
        $row = $conn->fetch_array();
    ...
    public static function editEval($info = array())
    {
        ...
        foreach ($info['Mark'] as $key => $mark) {
            $query = "UPDATE survey_eval SET Mark=$mark, Comment='" . $info['Comment'][$key] . "' WHERE EvalID=$key";
            $conn->query($query);
        }
    }
    ...
    public static function setSection()
    {
        ...
        switch ($_GET['action']) {
            ...
            case 'new_section':
                ...
                $conn->query("INSERT INTO survey_eval_sections(EvalFormDraftID,Name, Pondere, Priority, Status,CreateDate)
            ...
            case 'edit_section':
                ...
                $Priority = (int)$_GET['Priority'];
                $Status = (int)$_GET['Status'];
                $conn->query("UPDATE survey_eval_sections SET
		                			    Name       = '$Name',
		                			   	Pondere    = '$Pondere',
		                			    Priority   = '$Priority',
		                			    Status     = '$Status',
            ...
            case 'del_section':
                $SectionID = (int)$_GET['SectionID'];
                $conn->query("DELETE FROM survey_eval_sections WHERE SectionID = $SectionID");
                break;
        }
    }
    ...
    public static function getFormDraftSections($EvalFormDraftID)
    {
        ...
        $sections = array();
        $conn->query("SELECT * FROM survey_eval_sections WHERE EvalFormDraftID='$EvalFormDraftID'ORDER BY Priority ASC");
        while ($row = $conn->fetch_array()) {
            $sections[$row['SectionID']] = $row;
        }

        foreach ($sections as $sectionID => $section) {
            $query = "SELECT * FROM survey_eval_questions WHERE SectionID='$sectionID'";
            $conn->query($query);
            while ($row = $conn->fetch_array()) {
    ...
    public function addEvalFormDraft($info = array())
    {
        ...
        $conn->query("INSERT INTO survey_eval_forms_draft(UserID, FormName, CreateDate)
    ...
    public function setEvalToPerson($info = array())
    {
        ...
        $conn->query("INSERT INTO survey_eval_forms_groups(EvalFormID, PersonID)
	                      VALUES(
	                      '$EvalFormID',
	                      '{$info['PersonID']}')
	                      ");
        ...
        foreach ($info['EvaluatorID'] as $EvaluatorID) {
            ...
            $conn->query("INSERT INTO survey_eval_forms(UserID, PersonID, EvaluatorID, ManagerID, EvalFormDraftID, RelationID, EvalFormKey, FormName, FunctionID, StartDate, EndDate, CreateDate)
                      ...
                      '{$info['RelationID']}',
                      '" . md5(date('s')) . "',
                      (SELECT FormName FROM survey_eval_forms_draft WHERE EvalFormDraftID={$info['EvalFormDraftID']}),
	                      '{$info['FunctionID']}' ,
                      '{$info['StartDate']}',
                      '{$info['EndDate']}',
                      CURRENT_TIMESTAMP)");
            ...
            $conn->query("INSERT INTO survey_eval(UserID, EvalQuestionID, EvalFormID)
							SELECT {$_SESSION['USER_ID']}, EvalQuestionID, $EvalFormID
                            FROM survey_eval_forms_draft a
                            LEFT JOIN survey_eval_questions b ON a.EvalFormDraftID=b.EvalFormDraftID
    ...
    public function editEvalFormDraft($EvalFormDraftID, $info = array())
    {
        ...
        $conn->query("UPDATE survey_eval_forms_draft SET
        				UserID={$_SESSION['USER_ID']},
    ...
    public function deleteEvalFormDraft($EvalFormDraftID)
    {
        ...
        $conn->query("DELETE FROM survey_eval_forms_draft WHERE EvalFormDraftID=$EvalFormDraftID
        				AND NOT EXISTS(SELECT EvalFormID from survey_eval_forms WHERE EvalFormDraftID=$EvalFormDraftID)");
        if (!$conn->get_affected_rows()) {
            echo "<body onload=\"alert('Nu se poate sterge acest template de evaluare deoarece este deja utilizat!'); window.location.href = './?m=surveys-eval&o=formsDraft';\"></body>";
            exit;
        }
    }
    ...
    public function getEvalFormDraft($EvalFormDraftID)
    {
        ...
        $query = "SELECT * FROM survey_eval_forms_draft
                  WHERE  EvalFormDraftID = {$EvalFormDraftID} AND
                         (UserID = {$_SESSION['USER_ID']} OR '{$_SESSION['USER_SETTINGS'][25]}' > 1 OR {$_SESSION['USER_ID']} = 1)";
        $conn->query($query);
