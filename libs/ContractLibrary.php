...
class ContractLibrary
{
    ...
    public static function getAllDocuments($action = '')
    {
        ...
        $condbase = "('{$_SESSION['USER_RIGHTS3'][21][1][3]}' > 0 AND
	             (('{$_SESSION['USER_RIGHTS2'][21][1]}' = 1 AND a.UserID = {$_SESSION['USER_ID']} AND a.PID = '{$_SESSION['PERS']}') OR
	             '{$_SESSION['USER_RIGHTS2'][21][1]}' > 1))
	             OR 
		     {$_SESSION['USER_ID']} = 1";
        $condrw = "('{$_SESSION['USER_RIGHTS2'][21][1]}' = 3 AND a.UserID = {$_SESSION['USER_ID']} AND a.PID = '{$_SESSION['PERS']}')
	             OR
	             '{$_SESSION['USER_RIGHTS3'][21][1][3]}' = 2
		     OR 
		     {$_SESSION['USER_ID']} = 1";

        $query = "SELECT COUNT(*) AS total FROM contract_doc a WHERE ContractID = $ContractID AND ($condbase) $cond";
        $conn->query($query);
        ...
        $query = "SELECT a.*, DATE_FORMAT(a.CreateDate, '%d.%m.%Y') AS data, CASE WHEN $condrw THEN 1 ELSE 0 END AS rw 
                  FROM   contract_doc a
                  WHERE  ContractID = $ContractID AND ($condbase) $cond
        ...
        while ($row = $conn->fetch_array()) {
            ...
            $row['curr_filename'] = 'docscontr/' . md5($row['ContractID']) . '/' . str_replace(' ', '-', $row['FileName']);
            $docs[$row['DocID']] = $row;
        }
    ...
    public function addDoc($info = array(), $fileinfo = array(), $doc_dir = '')
    {
        ...
        $conn->query("INSERT INTO contract_doc(UserID, PID, ContractID, CreateDate, LastUpdateDate, " . implode(", ", array_keys($info)) . ")
                      VALUES({$_SESSION['USER_ID']}, '{$_SESSION['PERS']}', '{$_GET['ContractID']}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '" . implode("', '", $info) . "')");
    }
    ...
    public function editDoc($info = array(), $fileinfo = array(), $doc_dir = '')
    {
        ...
        $conn->query("UPDATE contract_doc a SET $update LastUpdateDate = CURRENT_TIMESTAMP WHERE DocID = {$this->DocID} AND ($condrw)");
    }
    ...
    public function getDoc()
    {
        ...
        $query = "SELECT a.*, CASE WHEN $condrw THEN 1 ELSE 0 END AS rw FROM contract_doc a WHERE a.DocID = {$this->DocID} AND ($condbase)";
        $conn->query($query);
    ...
    public function delDoc()
    {
        ...
        $condrw = "('{$_SESSION['USER_RIGHTS2'][21][1]}' = 3 AND UserID = {$_SESSION['USER_ID']} AND PID = '{$_SESSION['PERS']}')
	             OR
	             '{$_SESSION['USER_RIGHTS3'][21][1][3]}' = 2
		     OR 
		     {$_SESSION['USER_ID']} = 1";

        $query = "DELETE FROM contract_doc WHERE DocID = {$this->DocID} AND ($condrw)";
        $conn->query($query);
    }
}
