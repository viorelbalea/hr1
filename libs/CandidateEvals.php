...
class CandidateEvals extends ConfigData
{
    ...
    public static function getAllPersons($action = '')
    {
        ...
        $query = "SELECT COUNT(*) AS total
                   FROM   candidates_internal a
                          ...
                          LEFT JOIN address_city d ON d.CityID = b.CityID" . (!empty($_GET['CityID']) ? " AND d.CityID = " . (int)$_GET['CityID'] : "") . "
        ...
        $query = "SELECT a.*, d.CityName, e.DistrictName,
	              ...
	              FROM   candidates_internal a
                       ...
                       LEFT JOIN address_district e ON e.DistrictID = d.DistrictID" . (!empty($_GET['DistrictID']) ? " AND e.DistrictID = " . (int)$_GET['DistrictID'] : "") . "
                       LEFT JOIN users g ON a.UserID = g.UserID
                       LEFT JOIN candidates_eval_forms i ON a.PersonID=i.PersonID
				  WHERE  ((a.UserID = {$_SESSION['USER_ID']} OR a.PID = '{$_SESSION['PERS']}' OR '{$_SESSION['USER_RIGHTS2'][32][1]}' > 1 OR {$_SESSION['USER_ID']} = 1)) $cond
    ...
    public static function getAllPersonsEvaluator($action = '')
    {
        ...
        $query = "SELECT COUNT(*) AS total
                   FROM   persons a
                          ...
                          INNER JOIN candidates_eval_forms i ON a.PersonID=i.PersonID
    ...
    public static function cloneEvalFormDraft($EvalFormDraftID)
    {
        ...
        $query = "INSERT INTO candidates_eval_forms_draft (UserID,FormName,CreateDate)
       				SELECT UserID,CONCAT_WS('','$txClone',FormName),CURRENT_TIMESTAMP FROM candidates_eval_forms_draft
        ...
        $query = "SELECT * FROM candidates_eval_sections a
    			WHERE  a.EvalFormDraftID=$EvalFormDraftID";
        $r1 = $conn->query($query);
        while ($row = $conn->fetch_array($r1)) {
            // Insert new sections
            $query2 = "INSERT INTO candidates_eval_sections SET
            ...
            $query3 = "SELECT * FROM candidates_eval_questions a
	    			WHERE  a.EvalFormDraftID=$EvalFormDraftID
	    			AND a.SectionID='{$row['SectionID']}'";
            $r3 = $conn->query($query3);
            while ($row3 = $conn->fetch_array($r3)) {
                $query4 = "INSERT INTO candidates_eval_questions SET
    ...
    public static function getEvalFormsDraft()
    {
        ...
        $query = "SELECT COUNT(*) AS total
				FROM candidates_eval_forms_draft a
    			LEFT JOIN candidates_eval_forms c ON a.EvalFormDraftID=c.EvalFormDraftID
        ...
        $query = "SELECT a.EvalFormDraftID, a.FormName, a.CreateDate, DATE_FORMAT(a.CreateDate, '%d.%m.%Y') AS CreateDate, (SELECT COUNT(*) FROM candidates_eval_forms WHERE EvalFormDraftID=a.EvalFormDraftID) AS AssignedForms 
    			FROM candidates_eval_forms_draft a
    			LEFT JOIN candidates_eval_forms c ON a.EvalFormDraftID=c.EvalFormDraftID
    			WHERE (a.UserID = {$_SESSION['USER_ID']} OR '{$_SESSION['USER_SETTINGS'][25]}' > 1 OR {$_SESSION['USER_ID']} = 1) $cond
    ...
    public static function getPersonsByFormDraft($EvalFormDraftID)
    {
        ...
        $query = "SELECT COUNT(b.EvalFormID) AS EvalSum,c.PersonID,c.FullName FROM candidates_eval_forms_draft a
				INNER JOIN candidates_eval_forms b ON a.EvalFormDraftID=b.EvalFormDraftID
				INNER JOIN candidates_internal c ON c.PersonID=b.PersonID
				WHERE a.EvalFormDraftID=$EvalFormDraftID
				GROUP BY c.PersonID";
        $conn->query($query);
        while ($row = $conn->fetch_array()) {
    ...
    public static function addEvalQuestion($EvalQuestionID)
    {
        ...
        if ($EvalQuestionID > 0 && $_GET['action'] == 'del_question') {
            $conn->query("DELETE FROM candidates_eval_questions
                          WHERE  EvalQuestionID = $EvalQuestionID AND
                                 NOT EXISTS(SELECT EvalQuestionID FROM candidates_eval WHERE EvalQuestionID = $EvalQuestionID)");
            if (!$conn->get_affected_rows()) {
                $conn->query("SELECT EvalFormDraftID FROM candidates_eval_questions WHERE EvalQuestionID = $EvalQuestionID");
                $row = $conn->fetch_array();
            ...
            //exit;
            if ($EvalQuestionID > 0) {
                $conn->query("UPDATE candidates_eval_questions SET Question = '$Question', Pondere = '$Pondere', SectionID=$SectionID WHERE EvalQuestionID = $EvalQuestionID");
            } else {
                $conn->query("INSERT INTO candidates_eval_questions(UserID,EvalFormDraftID, Question, Pondere, SectionID) VALUES('{$_SESSION['USER_ID']}', '{$_GET['EvalFormDraftID']}','$Question', '$Pondere', $SectionID)");
            }
            if ($conn->errno == 1062) {
    ...
    public static function getEvalQuestions($EvalFormDraftID)
    {
        ...
        global $conn;
        $query = "SELECT *,a.Pondere AS Pondere FROM candidates_eval_questions a
    			INNER JOIN candidates_eval_sections b ON a.SectionID=b.SectionID
    			WHERE  a.EvalFormDraftID=$EvalFormDraftID
    ...
    public static function getEvalFormsPersonal()
    {
        ...
        if (!isset($_SESSION['USER_RIGHTS2'][25][1]))
            $_SESSION['USER_RIGHTS2'][25][1] = NULL;
        if (!isset($_SESSION['USER_RIGHTS2'][32][1]))
            $_SESSION['USER_RIGHTS2'][32][1] = NULL;

        $query = "SELECT COUNT(*) AS total FROM candidates_eval_forms a
        ...
        $query = "SELECT *, d.FullName as EvaluatedName FROM candidates_eval_forms a
    			LEFT JOIN functions b ON a.FunctionID=b.FunctionID
    			LEFT JOIN persons c ON a.EvaluatorID=c.PersonID
    			LEFT JOIN candidates_internal d ON a.PersonID=d.PersonID
    			WHERE  1=1
    			AND (a.ManagerID='{$_SESSION['PERS']}' OR a.UserID = {$_SESSION['USER_ID']} OR a.PersonID = '{$_SESSION['PERS']}' OR '{$_SESSION['USER_RIGHTS2'][32][1]}' > 1 OR {$_SESSION['USER_ID']} = 1) $cond
    			GROUP BY EvalFormKey
    			ORDER  BY $order_by $asc_or_desc " .
            (in_array($action, array('export', 'print_all')) ? '' : "LIMIT  " . ($page - 1) * $res_per_page . ", " . $res_per_page);
        $conn->query($query);
        ...
        foreach ($evalForms as $evalFormID => $evalForm) {
            $query = "SELECT * FROM candidates_eval_forms a
    			LEFT JOIN persons c ON a.EvaluatorID=c.PersonID
    ...
    public static function getAllPersonsByFormsDraft()
    {
        ...
        $query = "SELECT * FROM candidates_eval_forms a
    			LEFT JOIN candidates_internal c ON a.PersonID=c.PersonID
    			GROUP BY c.PersonID";
        $conn->query($query);
        while ($row = $conn->fetch_array()) {
            $evalForms[$row['EvalFormID']] = $row;
    ...
    public static function completeEvalForm($CStatus, $EvalFormID)
    {
        ...
        $query = "SELECT Mark,q.Pondere AS Pondere,s.Pondere AS SPondere,s.SectionID, Mark*q.Pondere AS Nominator
					FROM candidates_eval e
					      LEFT JOIN candidates_eval_questions q ON q.EvalQuestionID=e.EvalQuestionID
					      LEFT JOIN candidates_eval_sections s ON q.SectionID=s.SectionID
        ...
        //update eval form
        $query = "UPDATE candidates_eval_forms SET Completed=$CStatus, SelfWeighted=$weighted, CompletedDate=CURRENT_TIMESTAMP WHERE EvalFormID=$EvalFormID";
        $conn->query($query);
    }
    ...
    public static function deleteEvalForm($EvalFormID)
    {
        ...
        $query = "DELETE FROM candidates_eval_forms WHERE EvalFormID=$EvalFormID";
        $conn->query($query);

        $query = "DELETE FROM candidates_eval WHERE EvalFormID=$EvalFormID";
        $conn->query($query);
    }
    ...
    public static function getEval($EvalFormID)
    {
        ...
        $query = "SELECT *,d.Name AS SectionName, b.Pondere AS Pondere, d.Pondere AS SPondere, c.PersonExternEmail FROM candidates_eval a
       			LEFT JOIN candidates_eval_questions b ON a.EvalQuestionID=b.EvalQuestionID
       			LEFT JOIN candidates_eval_forms c ON c.EvalFormID=a.EvalFormID
       			LEFT JOIN candidates_eval_sections d ON b.SectionID=d.SectionID
        ...
        while ($row = $conn->fetch_array()) {
            ...
            $eval['PersonExternEmail'] = $row['PersonExternEmail'];
            $eval['EvaluatorExternEmail'] = $row['EvaluatorExternEmail'];
        }
    ...
    public static function getPersonByForm($EvalFormID)
    {
        ...
        $query = "SELECT b.PersonID FROM candidates_eval a
       			LEFT JOIN candidates_eval_forms b ON b.EvalFormID=a.EvalFormID
    ...
    public static function getEvaluatorByForm($EvalFormID)
    {
        ...
        $query = "SELECT b.EvaluatorID FROM candidates_eval a
       			LEFT JOIN candidates_eval_forms b ON b.EvalFormID=a.EvalFormID
    			WHERE  b.EvalFormID=$EvalFormID
    			LIMIT 1";
        $conn->query($query);
...
########################    Evaluations Forms Personal  #######################################
    ...
    public static function getCompletedStatus($EvalFormID)
    {
        ...
        $conn->query("SELECT Completed FROM candidates_eval_forms WHERE EvalFormID=$EvalFormID");
        $row = $conn->fetch_array();
    ...
    public static function editEval($info = array())
    {
        ...
        foreach ($info['Mark'] as $key => $mark) {
            $query = "UPDATE candidates_eval SET Mark=$mark, Comment='" . $info['Comment'][$key] . "' WHERE EvalID=$key";
            $conn->query($query);
        }
    }
    ...
    public static function setSection()
    {
        ...
        switch ($_GET['action']) {
            ...
            case 'new_section':
                ...
                $conn->query("INSERT INTO candidates_eval_sections(EvalFormDraftID,Name, Pondere, Priority, Status,CreateDate)
            ...
            case 'edit_section':
                ...
                $Priority = (int)$_GET['Priority'];
                $Status = (int)$_GET['Status'];
                $conn->query("UPDATE candidates_eval_sections SET
		                			    Name       = '$Name',
		                			   	Pondere    = '$Pondere',
		                			    Priority   = '$Priority',
		                			    Status     = '$Status',
            ...
            case 'del_section':
                $SectionID = (int)$_GET['SectionID'];
                $conn->query("DELETE FROM candidates_eval_sections WHERE SectionID = $SectionID");
                break;
        }
    }
    ...
    public static function getFormDraftSections($EvalFormDraftID)
    {
        ...
        $sections = array();
        $conn->query("SELECT * FROM candidates_eval_sections WHERE EvalFormDraftID='$EvalFormDraftID'ORDER BY Priority ASC");
        while ($row = $conn->fetch_array()) {
            $sections[$row['SectionID']] = $row;
        }

        foreach ($sections as $sectionID => $section) {
            $query = "SELECT * FROM candidates_eval_questions WHERE SectionID='$sectionID'";
            $conn->query($query);
            while ($row = $conn->fetch_array()) {
    ...
    public static function getCandidates()
    {
        ...
        global $conn;
        $cond = '';
        $query = "SELECT a.PersonID, a.FullName FROM candidates_internal a
    					WHERE 1=1 $cond
    					ORDER BY FullName";
        $conn->query($query);
        while ($row = $conn->fetch_array()) {
    ...
    public function addEvalFormDraft($info = array())
    {
        ...
        $conn->query("INSERT INTO candidates_eval_forms_draft(UserID, FormName, CreateDate)
    ...
    public function setEvalToPerson($info = array())
    {
        ...
        if ((empty($info['PersonID']) || $info['PersonID'] <= 0) && empty($info['isPersonExtern'])) {
            throw new Exception(Message::getMessage('NO_SEL_PERSON'));
        }
        if ((empty($info['EvaluatorID']) || $info['EvaluatorID'] <= 0) && empty($info['isEvaluatorExtern'])) {
        ...
        if (!empty($info['isPersonExtern']))
            $personID = 0;
        else
            $personID = $info['PersonID'];


        if (!empty($info['EvaluatorID']) && empty($info['isEvaluatorExtern'])) {
            foreach ($info['EvaluatorID'] as $EvaluatorID) {
                // assign an eval form to a person for every evaluator
                $query = "INSERT INTO candidates_eval_forms(UserID, PersonID, isPersonExtern,PersonExternEmail, EvaluatorID, isEvaluatorExtern, EvaluatorExternEmail, ManagerID, EvalFormDraftID, RelationID, EvalFormKey, FormName, FunctionID, StartDate, EndDate, CreateDate)
		                      VALUES({$_SESSION['USER_ID']},
		                      '$personID',
		                      '" . (int)$info['isPersonExtern'] . "',
		                      '{$info['PersonExternEmail']}',
		                      '$EvaluatorID',
		                      '{$info['isEvaluatorExtern']}',
		                      '{$info['EvaluatorExternEmail']}',
		                      (SELECT ManagerID FROM candidates_internal WHERE PersonID='$personID'),
		                      '{$info['EvalFormDraftID']}',
		                      '{$info['RelationID']}',
		                      '" . md5(date('s')) . "',
		                      (SELECT FormName FROM candidates_eval_forms_draft WHERE EvalFormDraftID={$info['EvalFormDraftID']}),
			                      '{$info['FunctionID']}' ,
		                      '{$info['StartDate']}',
		                      '{$info['EndDate']}',
		                      CURRENT_TIMESTAMP)";
                ...
                $conn->query("INSERT INTO candidates_eval_forms_groups(EvalFormID, PersonID)
			                      VALUES(
			                      '$EvalFormID',
			                      '$personID')
			                      ");
                ...
                $conn->query("INSERT INTO candidates_eval(UserID, EvalQuestionID, EvalFormID)
									SELECT {$_SESSION['USER_ID']}, EvalQuestionID, $EvalFormID
		                            FROM candidates_eval_forms_draft a
		                            LEFT JOIN candidates_eval_questions b ON a.EvalFormDraftID=b.EvalFormDraftID
		                            WHERE a.EvalFormDraftID={$info['EvalFormDraftID']}");
            }
        } else {
            $evaluatorEmails = array();
            if (!empty($info['EvaluatorExternEmail'])) {
                $evaluatorEmails = explode(';', $info['EvaluatorExternEmail']);
                //Utils::pa($evaluatorEmails);exit;

                foreach ($evaluatorEmails as $evaluatorEmail) {
                    // assign an eval form to a person for every evaluator
                    $query = "INSERT INTO candidates_eval_forms(UserID, PersonID, isPersonExtern,PersonExternEmail, EvaluatorID, isEvaluatorExtern, EvaluatorExternEmail, ManagerID, EvalFormDraftID, RelationID, EvalFormKey, FormName, FunctionID, StartDate, EndDate, CreateDate)
			                      VALUES({$_SESSION['USER_ID']},
			                      '$personID',
    ...
    public function editEvalFormDraft($EvalFormDraftID, $info = array())
    {
        ...
        $conn->query("UPDATE candidates_eval_forms_draft SET
        				UserID={$_SESSION['USER_ID']},
    ...
    public function deleteEvalFormDraft($EvalFormDraftID)
    {
        ...
        $conn->query("DELETE FROM candidates_eval_forms_draft WHERE EvalFormDraftID=$EvalFormDraftID
        				AND NOT EXISTS(SELECT EvalFormID from candidates_eval_forms WHERE EvalFormDraftID=$EvalFormDraftID)");
        if (!$conn->get_affected_rows()) {
    ...
    public function getEvalFormDraft($EvalFormDraftID)
    {
        ...
        $query = "SELECT * FROM candidates_eval_forms_draft
                  WHERE  EvalFormDraftID = {$EvalFormDraftID} AND
                         (UserID = {$_SESSION['USER_ID']} OR '{$_SESSION['USER_SETTINGS'][25]}' > 1 OR {$_SESSION['USER_ID']} = 1)";
        $conn->query($query);
