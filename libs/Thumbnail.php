...
###### CUM SE FOLOSESTE & SALVARE

$thumb=new Thumbnail(UPL_PATH."source.jpg");	        // Contructor and set source image file
$thumb->img_watermark='watermark.png';	    // [OPTIONAL] set watermark source file, only PNG format [RECOMENDED ONLY WITH GD 2 ]
$thumb->size(180,150);
$thumb->txt_watermark='MERGE';	    // [OPTIONAL] set watermark text [RECOMENDED ONLY WITH GD 2 ]
$thumb->txt_watermark_color='C4C4C3';	    // [OPTIONAL] set watermark text color , RGB Hexadecimal[RECOMENDED ONLY WITH GD 2 ]
$thumb->txt_watermark_font=8;	            // [OPTIONAL] set watermark text font: 1,2,3,4,5
$thumb->txt_watermark_Valing='TOP';   	    // [OPTIONAL] set watermark text vertical position, TOP | CENTER | BOTTOM
$thumb->txt_watermark_Haling='LEFT';       // [OPTIONAL] set watermark text horizonatal position, LEFT | CENTER | RIGHT
$thumb->txt_watermark_Hmargin=10;          // [OPTIONAL] set watermark text horizonatal margin in pixels
$thumb->txt_watermark_Vmargin=10; 		   // [OPTIONAL] set the biggest width and height for thumbnail
$thumb->process();   				        // generate image
$filename=$thumb->unique_filename ( UPL_PATH , 'sample3.jpg' , 'thumb');  // generate unique filename
$cpath=IMG_CACHE_PATH.$filename;
$status=$thumb->save($cpath);            	// save your thumbnail to file
if ($status) {
    echo('Thumbnail save as '.$cpath);
} else {
    echo('ERROR: '.$thumb->error_msg);
}
}
sas('img',IMG_CACHE_URL.$filename);
################################################

$img=resize_img(IMG_UPL_PATH.'source.jpg',900,451); //functia se afla in functions.images
###sas('img',$img);
...
class Thumbnail
{
    ...
    public static function resize_img($img_path, $x, $y)
    {
        //echo substr(sprintf('%o', fileperms(IMG_CACHE_PATH)), -4);
        $img = substr(strrchr($img_path, '/'), 1);            // Get the image name from the full path
        $img_arr = explode(".", $img);
        $img_name = $img_arr['0'];                            //Obtinem numele imaginii fara extensie
        $extension = strtolower($img_arr['1']);
        $new_img_name = $img_name . "_" . $x . "_" . $y . "." . $extension;        //cream o imagine ce are in nume si dimensiunea
        if (!file_exists(Config::IMG_CACHE_PATH . $new_img_name) && @fopen($img_path, "r"))              // Daca nu e deja acea imagine cu dim, cream una noua
        {
            $thumb = new Thumbnail($img_path);                      // Contructor and set source image file
            $thumb->size($x, $y);
            $thumb->process();                                // generate image
            $cache_path = Config::IMG_CACHE_PATH . $new_img_name;
            $status = $thumb->save($cache_path);                    // salvam imaginea in folderul de cache
        }
        // generate unique filename; if the name exists, there is creted a new random one that starts with thumb
        //$filename=$thumb->unique_filename ( IMG_UPL_PATH , $img_name , 'thumb');
        /*
        if ($status) {
        echo('Thumbnail save as '.$cache_path);
        } else {
        echo('ERROR: '.$thumb->error_msg);
        }
        */
        return Config::IMG_CACHE_URL . $new_img_name;
    }
...
/**
    ...
    function save($save = "")
    {
        ...
        if ($this->output_format == "PNG") { //PNG
            ...
            @imageGIF($this->img["des"], "$save", $this->quality);
        } else {
    ...
    function unique_filename($archive_dir, $filename, $file_prefix = '')
    {
        ...
        $destination_name = $file_prefix . "_" . $name . "." . $extension;
        //$destination_path = $archive_dir."/".$file_prefix."_".$name.".".$extension;

        while (file_exists($archive_dir . "/" . $destination_name)) {
            // if exist, add a random number to the file name
            srand((double)microtime() * 1000000); // random number inizializzation
            $destination_name = $file_prefix . "_" . $name . "_" . rand(0, 999999999) . "." . $extension;
            //$destination_path = $archive_dir."/".$file_prefix."_".$name."_".rand(0,999999999).".".$extension;
        }
