...
class PersonLibrary
{
    ...
    public static function getAllDocuments($action = '')
    {
        ...
        $condbase = "('{$_SESSION['USER_RIGHTS3'][1][1][14]}' > 0 AND
        ...
        $condrw = "('{$_SESSION['USER_RIGHTS2'][1][1]}' = 3 AND (a.UserID = {$_SESSION['USER_ID']} OR a.PersonID = '{$_SESSION['PERS']}' $condmng))
	             OR
	             '{$_SESSION['USER_RIGHTS3'][1][1][14]}' = 2
		     OR 
		     {$_SESSION['USER_ID']} = 1";

        $query = "SELECT COUNT(*) AS total FROM persons_doc a WHERE PersonID = $PersonID AND ($condbase) $cond";
        $conn->query($query);
        ...
        while ($row = $conn->fetch_array()) {
            ...
            $row['curr_filename'] = 'docspers/' . md5($row['UserID']) . '/' . md5($row['PersonID']) . '/' . str_replace(' ', '-', $row['FileName']);
            $docs[$row['DocID']] = $row;
        }
    ...
    public static function markRead($PersonID, $DocID)
    {
        global $conn;
        echo $query = "INSERT INTO persons_mandatory_reads (PersonID, DocID, CreateDate) 
					VALUES ('$PersonID','$DocID', CURRENT_TIMESTAMP)";
        $conn->query($query);
    }
    ...
    {
        global $conn;
        $query = "SELECT a.* FROM persons_doc a
						LEFT OUTER JOIN  persons_mandatory_reads b ON b.DocID=a.DocID
						WHERE a.MandatoryReading=1 
						AND a.PersonID = '$PersonID' AND b.PersonID IS NULL";
        $conn->query($query);
        while ($row = $conn->fetch_array()) {
            $row['curr_filename'] = 'docspers/' . md5($row['UserID']) . '/' . md5($row['PersonID']) . '/' . str_replace(' ', '-', $row['FileName']);
            $res[] = $row;
    ...
    public function addDoc($info = array(), $fileinfo = array(), $doc_dir = '')
    {
        ...
        $conn->query("INSERT INTO persons_doc(UserID, PersonID, CreateDate, LastUpdateDate, MandatoryReading, " . implode(", ", array_keys($info)) . ")
                      VALUES({$_SESSION['USER_ID']}, '{$_GET['PersonID']}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, '$mandatoryReading', '" . implode("', '", $info) . "')");
        if ($conn->errno == 1062) {
    ...
    public function editDoc($info = array(), $fileinfo = array(), $doc_dir = '')
    {
        ...
        $conn->query("UPDATE persons_doc a SET $update MandatoryReading='$mandatoryReading', LastUpdateDate = CURRENT_TIMESTAMP WHERE DocID = {$this->DocID} AND ($condrw)");
        if ($conn->errno == 1062) {
            throw new Exception(Message::getMessage('DUPLICATE_CIF'));
        }
    }
    ...
    public function getDoc()
    {
        ...
        $query = "SELECT a.*, CASE WHEN $condrw THEN 1 ELSE 0 END AS rw FROM persons_doc a WHERE a.DocID = {$this->DocID} AND ($condbase)";
        $conn->query($query);
    ...
    public function delDoc()
    {
        ...
        $condrw = "('{$_SESSION['USER_RIGHTS2'][1][1]}' = 3 AND (UserID = {$_SESSION['USER_ID']} OR PersonID = '{$_SESSION['PERS']}' $condmng))
	             OR
	             '{$_SESSION['USER_RIGHTS3'][1][1][14]}' = 2
		     OR 
		     {$_SESSION['USER_ID']} = 1";

        $query = "DELETE FROM persons_doc WHERE DocID = {$this->DocID} AND ($condrw)";
        $conn->query($query);
    }
